...
class EditLodgePage extends StatefulWidget {
  final Lodge? existingLodge;
  final Contact? existingContact;
  final List<Service>? existingServices;
  final List<GalleryImage>? existingImages;
  final List<Information>? existingInformation;
  ...
  @override
  State<EditLodgePage> createState() => _EditLodgePageState();
}

class _EditLodgePageState extends State<EditLodgePage> {
  ...
  void initState() {
    ...
    if (widget.existingLodge != null) {
      _titleController.text = widget.existingLodge!.title;
      _descriptionController.text = widget.existingLodge!.description;
      _logoController.text = widget.existingLodge!.logo;
    }

    if (widget.existingContact != null) {
      _telefonoController.text = widget.existingContact!.telefono;
      _websiteController.text = widget.existingContact!.website;
      _correoController.text = widget.existingContact!.correo;
      _ubicacionController.text = widget.existingContact!.ubicacion;
    }

    if (widget.existingServices != null) {
      _services.addAll(widget.existingServices!.map((s) => s.name));
    }

    if (widget.existingImages != null) {
      _images.addAll(widget.existingImages!
          .map((img) => {'url': img.url, 'pie': img.footer}));
    }

    if (widget.existingInformation != null) {
      _information.addAll(widget.existingInformation!.map((info) => info.info));
    }
  }
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<LodgeBloc, LodgeState>(
      ...
      builder: (context, state) {
        ...
        context.read<LodgeBloc>().add(UpdateLodgeEvent(
          lodge: lodge,
          contact: contact,
          services: _services.map((s) => Service(name: s)).toList(),
          images: _images
              .map((img) => GalleryImage(url: img['url']!, footer: img['pie']!))
              .toList(),
          information: _information
              .map((info) => Information(info: info))
              .toList(),
        ));*/
                                        }
                                      },
